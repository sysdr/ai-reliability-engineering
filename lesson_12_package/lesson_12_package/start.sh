#!/usr/bin/env bash
# Day 12 - start.sh: install deps, build on Docker Desktop, run, test, verify.
set -euo pipefail
...
pkill -f "python -m http.server ${DASHBOARD_PORT}" 2>/dev/null || true
...
if [ -n "$DOCKER_BIN" ]; then
    ...
    if "$DOCKER_BIN" buildx version &> /dev/null; then
        ...
        DOCKER_BUILDKIT=1 "$DOCKER_BIN" build -t "$IMAGE_NAME" "$BUILD_CONTEXT"
    fi
...
if "retrieval precision" not in html or "classifier recall" not in html or "multi-source synthesis" not in html:
    raise SystemExit("ERROR: dashboard missing failure-category breakdown sections")
if "By failure category" not in html:
    raise SystemExit("ERROR: dashboard missing category section heading")
print(f"Dashboard OK: {passed}/{total} handled correctly, {failed} known gaps ({rate})")
PY
else
