#!/usr/bin/env bash
# Day 7 - start.sh: install deps, build, run, test, verify.
...
IMAGE_NAME="day7-synthesis"
CONTAINER_NAME="day7-synthesis-run"
...
assert "Day 7" in html and "refund policy" in html.lower(), "demo query missing from dashboard"
assert "metric-queries" in html and "metric-synthesized" in html
assert "metric-skipped" in html and "metric-top-retrieval" in html
assert "metric-entity-matches" in html and "metric-citations" in html
...
assert initial["synthesized"] > 0, initial
assert initial["skipped"] > 0, initial
assert initial["citations"] > 0, initial
assert initial["entity_matches"] > 0, initial
assert initial["entity_mismatches"] > 0, initial
assert initial["avg_retrieval_score"] > 0, initial
assert initial["top_retrieval_score"] > 0, initial
assert any(
...
print(
    ...
    f"synthesized={initial['synthesized']} skipped={initial['skipped']} "
    f"citations={initial['citations']} mismatches={initial['entity_mismatches']} "
    f"top_retrieval={initial['top_retrieval_score']}"
)
PY
...
assert first["top_retrieval_score"] > 0, first
assert first["synthesized"] > 0, first
assert first["avg_retrieval_score"] > 0, first
assert first["citations"] > 0, first
...
changed = (
    ...
    or third["top_retrieval_score"] != first["top_retrieval_score"]
    or third["avg_retrieval_score"] != first["avg_retrieval_score"]
    or third["entity_mismatches"] != first["entity_mismatches"]
    or third["citations"] != first["citations"]
