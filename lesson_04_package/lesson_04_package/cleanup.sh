#!/usr/bin/env bash
# Day 4 - cleanup.sh: stop containers and remove unused Docker resources.
set -euo pipefail
...
echo "Removed venv, caches, pyc, node_modules, dashboard.html, and Istio artifacts (if present)."
...
echo ""
echo "== Stopping and removing Day 4 containers =="
for name in day4-hybrid-search-run day4-hybrid-search; do
...
# Also stop/remove any containers started from day4-hybrid-search images
while read -r cid; do
    [ -z "$cid" ] && continue
    docker stop "$cid" >/dev/null 2>&1 || true
    docker rm -f "$cid" >/dev/null 2>&1 || true
    echo "Removed container id: $cid"
done < <(docker ps -aq --filter ancestor=day4-hybrid-search 2>/dev/null || true)

echo ""
echo "== Removing Day 4 images =="
docker rmi -f day4-hybrid-search 2>/dev/null || true
