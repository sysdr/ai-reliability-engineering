#!/usr/bin/env bash
# Day 9 - start.sh: install deps, build, run, test, verify.
...
echo ""
echo "== Validating dashboard metrics (non-zero Day 9 formatter scores) =="
python - <<'PY'
...
assert "Day 9" in html and "ResponseFormatter" in html, "Day 9 title missing"
assert "RAW DRAFT" in html and "DELIVERED TO USER" in html, "raw vs delivered cards missing"
assert "refund policy" in html.lower(), "demo query missing from dashboard"
assert "metric-queries" in html and "metric-formatted" in html
assert "metric-approved-delivered" in html and "metric-rejected-blocked" in html
assert "metric-out-of-scope" in html and "metric-leak-checks-passed" in html
...
assert initial["formatted"] > 0, initial
assert initial["approved_delivered"] > 0, initial
assert initial["rejected_blocked"] > 0, initial
assert initial["out_of_scope"] > 0, initial
assert initial["approved_fallback"] > 0, initial
assert initial["leak_checks_passed"] > 0, initial
assert initial["raw_vs_delivered_diff"] > 0, initial
assert initial["distinct_outcomes"] >= 4, initial
assert any(row.get("status") == "rejected" for row in initial["results"]), initial
assert any(
    row.get("is_corrupted") and row.get("leak_clean") for row in initial["results"]
), initial
print(
    f"Dashboard OK: run=#{initial['run_id']} queries={initial['queries']} "
    f"formatted={initial['formatted']} approved={initial['approved_delivered']} "
    f"rejected_blocked={initial['rejected_blocked']} oos={initial['out_of_scope']} "
    f"leak_checks={initial['leak_checks_passed']} raw≠delivered={initial['raw_vs_delivered_diff']}"
)
PY
...
assert first["formatted"] > 0, first
assert first["approved_delivered"] > 0, first
assert first["rejected_blocked"] > 0, first
assert first["leak_checks_passed"] > 0, first
assert first["raw_vs_delivered_diff"] > 0, first
assert first["approved_fallback"] > 0, first
assert first["out_of_scope"] > 0, first
assert first["distinct_outcomes"] >= 4, first
...
changed = (
    ...
    or third["primary_status"] != first["primary_status"]
    or third["approved_delivered"] != first["approved_delivered"]
    or third["rejected_blocked"] != first["rejected_blocked"]
    or third["out_of_scope"] != first["out_of_scope"]
    or third["approved_fallback"] != first["approved_fallback"]
    or third["leak_checks_passed"] != first["leak_checks_passed"]
...
print(
    ...
    f"primary {first['primary_status']} -> {third['primary_status']}"
)
PY
...
echo "Open the live UI (raw draft vs delivered, metrics auto-refresh) at:"
if command -v docker &>/dev/null; then
